...
const generateAppFolder = (option: AppOption): string | boolean => {
  ...
  if (option.setAuth) {
    ...
    writeFileSync(`${option.appName}/auth.json`, authJSON, {
      spaces: 4,
      EOL: '\r\n'
    });
  }
  ...
  if (option.useWebpack) {
    ...
    writeFileSync(`${option.appName}/.babelrc`, babelJSON, {
      spaces: 4,
      EOL: '\r\n'
    });
  ...
  if (option.useCybozuLint) {
    ...
    packageJSON.scripts[
      `lint-${option.appName}`
    ] = `eslint ${option.appName}/ --ext ${lintedExtension}`;
    packageJSON.scripts[
      `lint-${option.appName}-fix`
    ...
    const eslintRcTemplete = buildEslintRcTemplate(option as EslintRcParams);
    writeFileSyncFS(`${option.appName}/.eslintrc.js`, eslintRcTemplete);
    spawnSync(
      'npx',
      ['prettier', '--write', `${option.appName}/.eslintrc.js`],
      { stdio: 'inherit' }
    );
  }

  writeFileSync(`${option.appName}/config.json`, manifestJSON, {
    spaces: 4,
    EOL: '\r\n'
  });
  if (option.entry) {
